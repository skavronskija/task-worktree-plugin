...
4. On confirm, `WorktreeService.createWorktree` shells out to `git worktree add` via `GeneralCommandLine` + `CapturingProcessHandler`, using `GitExecutableManager.getPathToGit(project)`. On success, `.idea/` is copied from the main repo root into the new worktree so the IDE opens with the same project config, then `ProjectUtil.openOrImport` opens it.

Key invariants worth knowing before editing:

- **Worktrees live in a sibling `worktrees/` directory** next to the repo root (`repoPath.getParent().resolve("worktrees")`). Folder defaults to `<repoName>-<taskId>`.
- **Branch reuse vs. create**: `createWorktree` checks `repo.getBranches().findLocalBranch(branchName)`. If it exists, `git worktree add <path> <branch>`. Otherwise `git worktree add -b <branch> <path> <base>` where base is the current branch (or `HEAD`).
- **Branch naming** goes through `WorktreeNaming.suggestBranchName`, which reflects into `TaskManagerImpl.suggestBranchName(task)` to respect the user's "Feature branch name format" template from the Tasks settings, then runs `GitRefNameValidator.cleanUpBranchName`. Fallback to `task.getPresentableId()` if the cast or call fails.
